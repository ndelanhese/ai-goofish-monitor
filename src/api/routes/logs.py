...
async def get_logs(
    ...
    """Get log content (incremental read)"""
    if task_id is None:
        return JSONResponse(content={
            "new_content": "Please select a task to view its logs.",
            "new_pos": 0
        })
    ...
    if not task:
        return JSONResponse(status_code=404, content={
            "new_content": "Task does not exist or has been deleted.",
            "new_pos": 0
        })
...
async def get_logs_tail(
    ...
    """Get log tail content (line-based pagination)"""
    if task_id is None:
        return JSONResponse(content={
            "content": "",
            "has_more": False,
...
async def clear_logs(
    ...
    """Clear the log file"""
    if task_id is None:
        return {"message": "No task specified; cannot clear logs."}

    task = await task_service.get_task(task_id)
    if not task:
        return {"message": "Task does not exist or has been deleted."}

    log_file_path = resolve_task_log_path(task_id, task.task_name)

    if not os.path.exists(log_file_path):
        return {"message": "Log file does not exist; nothing to clear."}
