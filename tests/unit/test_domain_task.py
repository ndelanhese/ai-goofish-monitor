...
def test_legacy_keyword_groups_are_flattened_to_keyword_rules():
    task = Task(
        ...
        keyword_rule_groups=[
            {"name": "Group 1", "include_keywords": ["a7m4", "验货宝"], "exclude_keywords": ["瑕疵"]},
            {"name": "Group 2", "include_keywords": ["全画幅", "a7m4"], "exclude_keywords": ["拆修"]},
        ],
        is_running=False,
    )
...
def test_generate_request_enables_image_analysis_by_default():
    req = TaskGenerateRequest(
        task_name="Sony A7M4",
        keyword="sony a7m4",
        description="Only evaluate body condition and seller credit.",
        decision_mode="ai",
    )
...
def test_generate_request_requires_state_file_for_fixed_account_strategy():
    try:
        ...
        assert "In fixed account mode, an account must be selected." in str(exc)
        return

    raise AssertionError("Fixed account mode should require account_state_file")
